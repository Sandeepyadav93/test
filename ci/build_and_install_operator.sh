...
make namespace
cd $HOME
git clone https://github.com/openstack-k8s-operators/$1.git 
cd $HOME/$1
...
export $(echo ${1^^} | cut -d '-' -f1)_IMG=default-route-openshift-image-registry.apps-crc.testing/openstack/$1-index:v0.0.1

echo $(echo ${1^^} | cut -d '-' -f1)_IMG
